...
{
	std::vector<ipPort_s> peerList;
	string hash;
	BitSwiftSelector *objThis;
};

struct t_arg_addpeer_s
{
	char *ip;
	int port;
	char *hash;
...
//typedef void (BitSwiftSelector::*THREADFUNCTIONPTR)(void * arg);
...
public:
	...
	void addpeers(std::vector<ipPort_s> ipPortList, string hash);
	void deletepeer(string ip, int port, string hash, int type = 100);
	void deletepeers(string hash, int type = 100);
	void getpeers(string hash, int type, std::vector<ipPort_s> &ip_port, int count = 0);
    void getPeersImmediately(string hash, int type, std::vector<ipPort_s> &ip_port, int count = 0);
protected:
	...
	void addPeersInDB(std::vector<ipPort_s> ipPortList, string hash);
	void addPeerInDB(string ip, int port, string hash);
	static void* entry(void *arg);
	static void* entryAddPeer(void *arg);
	
private:
    static char *m_myIp;
    GeoIP *m_gi;
    GeoIP *m_ASgi;
	//THREADFUNCTIONPTR threadFnPointer;
};
