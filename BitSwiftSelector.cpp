...
{
    //threadFnPointer = &BitSwiftSelector::addPeersInDB;
	CURL *curl;
    CURLcode res;
...
int BitSwiftSelector::getASN(const char* ipAddress)
{
		...
		{
		        string errorStirng = "Database is NULL \n";
                cout << errorStirng;
                return 0;
		}
}
...
void BitSwiftSelector::getPeersImmediately(string hash, int type, std::vector<ipPort_s> &ip_port, int number)
{
    int criteria; 
    string file_hash; 
    criteria = type;
...
void BitSwiftSelector::getpeers(string hash, int type, std::vector<ipPort_s> &ip_port, int number) 
{
    ...
    if (type == 0)
    {
        ...
        cout<<"================================================================================"<<endl;
        cout<<"Peers profiling based on score policy"<<endl;
...
void BitSwiftSelector::addpeers(std::vector<ipPort_s> ipPortList, string hash)
{
    ...
    thread_arg.peerList = ipPortList;
    thread_arg.hash = hash;
	thread_arg.objThis = this;
    
	/* Create worker thread to add peers addresses*/
	pthread_create(&pth,NULL, entry,(void *) &thread_arg);
	sleep(10);

}

void* BitSwiftSelector::entry(void *arg)
{
	struct thread_argument_s *my_data;
		my_data = (struct thread_argument_s *) arg;
	if (my_data)
	{
	    std::vector<ipPort_s> ipPortList = my_data->peerList;
		string hash = my_data->hash;
		if (my_data->objThis)
			my_data->objThis->addPeersInDB(ipPortList, hash);
	}
}

void BitSwiftSelector::addPeersInDB(std::vector<ipPort_s> ipPortList, string hash)
{
	//initialize();
	...
	getpeers(hash, 0 , ip_port_list);
	getpeers(hash, 1 , ip_port_list);
	getpeers(hash, 2 , ip_port_list);
}

void BitSwiftSelector::addpeer(string ip, int port, string hash)
{
	pthread_t pth;	// this is our thread identifier
    struct t_arg_addpeer_s thread_arg;
    thread_arg.ip = const_cast<char *>(ip.c_str());
	thread_arg.port = port;
    thread_arg.hash = const_cast<char *>(hash.c_str());
	thread_arg.objThis = this;
    
	/* Create worker thread to add peers addresses*/
	pthread_create(&pth,NULL, entryAddPeer,(void *)&thread_arg);
	sleep(2);
}

void* BitSwiftSelector::entryAddPeer(void *arg)
{
	struct t_arg_addpeer_s *my_data = NULL;
		my_data = (struct t_arg_addpeer_s *) arg;

    if (my_data)
    {
	    char * ip = my_data->ip;
	    int port = my_data->port;
	    char * hash = my_data->hash;

	    if (my_data->objThis)
		    my_data->objThis->addPeerInDB(ip, port, hash);
	}
}
void BitSwiftSelector::addPeerInDB(string ip, int port, string hash)
{
	std::cout << "Inside addPeerInDB" << std::endl;
        ...
        else
        {
            ...
            if (type == 100)
            {
				...
				if (query.execute())
				{
					...
					query.reset();
					query << "DELETE from ashop_table where ipaddr = (\""<<ip<<"\") AND port = (\""<<port<<"\") AND infohash = (\""<<hash<<"\")";
					if (!query.execute())
			        	cerr << "Error : peer is not present in table: " << query.error() << endl; 
				}
				else
        ...
        else
        {
            ...
            if (type == 100)
            {
				...
				if (query.execute())
				{
					...
					query.reset();
					query << "DELETE from ashop_table where infohash = (\""<<hash<<"\")";
					if (!query.execute())
			        	cerr << "Error : peer is not present in table: " << query.error() << endl; 
				}
				else
